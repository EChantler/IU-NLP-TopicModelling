...
if __name__ == "__main__":
    ...
    try:
        ...
        if data_run:
            ...
            # Load lemmatized transformer processed data (lemmatized cleaning for transformers)
            train_df = pd.read_csv(os.path.join(data_path, 'train_transformer_lem.csv'))
            val_df = pd.read_csv(os.path.join(data_path, 'val_transformer_lem.csv'))
            # Note: test_df is intentionally not loaded - reserved for final evaluation
            
            print(f"Loaded lemmatized transformer data from MLflow artifacts")
    ...
    mlflow.log_param('dataset_source', 'transformer_lemmatized')
    ...
    model_save_path = './results/final_transformer_model_lemmatized'
    ...
    train_report_path = './results/transformer_lemmatized_classification_report_train.txt'
    with open(train_report_path, 'w') as f:
        f.write("TRANSFORMER MODEL (LEMMATIZED) CLASSIFICATION REPORT (TRAINING SET)\n")
        f.write("="*70 + "\n")
        f.write(f"Model: {model_name}\n")
        f.write(f"Dataset: transformer_lemmatized (text only, no domain)\n")
        f.write(f"Training Accuracy: {acc_train:.4f}\n\n")
        f.write(train_report)
    
    # Save validation classification report
    val_report_path = './results/transformer_lemmatized_classification_report_val.txt'
    with open(val_report_path, 'w') as f:
        f.write("TRANSFORMER MODEL (LEMMATIZED) CLASSIFICATION REPORT (VALIDATION SET)\n")
        f.write("="*70 + "\n")
        f.write(f"Model: {model_name}\n")
        f.write(f"Dataset: transformer_lemmatized (text only, no domain)\n")
    ...
    mlflow.pytorch.log_model(
        model,
        artifact_path='transformer_model_lemmatized',
        signature=signature,
        registered_model_name='transformer_model_lemmatized'
    )
